...
export function MovementsScreen({
  ...
  const movimentacoesFiltradas = useMemo(() => {
    // Primeiro aplica a pesquisa
    let resultado = sections
      .map((section) => ({
        ...section,
    ...
    // Depois aplica o filtro selecionado
    switch (filtro) {
      case 'ontem':
        return resultado.filter(
          (section) => section.title === 'ONTEM'
        );

      case 'recentes': {
        // Junta todas as movimentações
        const todosItens = resultado.flatMap(
          (section) => section.data
        );

        // Pega as 5 movimentações mais recentes
        const cincoRecentes = todosItens.slice(0, 5);

        return cincoRecentes.length > 0
          ? [
              {
                title: 'RECENTES',
                data: cincoRecentes,
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  style={[
                    styles.opcaoFiltro,
                    filtro === item.key &&
                      styles.opcaoFiltroAtiva,
                  ]}
                  onPress={() =>
                    selecionarFiltro(item.key)
                  }
                >
                  <Text
                    style={[
